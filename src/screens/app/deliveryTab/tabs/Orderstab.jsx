...
import { Bike, LocationEdit } from "lucide-react-native";
...
const OrdersTab = ({ date }) => {
  ...
  useEffect(() => {
    if (orderlist?.orders) {
      if (page === 1) {
        ...
        setAllOrders((prev) => [...prev, ...orderlist.orders]);
      }
  ...
  const openReassign = (order, assignmentId) => {
    setModalConfig({
  ...
  const handleConfirmAssignment = async () => {
    ...
    const finalFee = deliveryFee.trim() !== "" ? parseFloat(deliveryFee) : null;
            ...
            <View style={styles.orderContent}>
              ...
              {assignment ? (
                ...
                <TouchableOpacity
                  ...
                  disabled={isSubmitting}
                >
                  <Text className="text-black font-semibold">Assign Rider</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
    ...
    <View style={styles.headerContainer}>
      <View style={styles.listHeader}>
        <Text style={styles.listHeaderText}>
          {orders.length} order{orders.length !== 1 ? "s" : ""} on this date
        </Text>
        {orders.length > 0 && (
          <TouchableOpacity
            onPress={
              selectedIds.size === orders.length ? clearSelection : selectAll
            }
          >
            <Text
              style={[styles.selectAllText, { color: theme.textSecondary }]}
            >
  ...
  useMemo(
    () =>
      StyleSheet.create({
        ...
        datePickerStyle: { fontSize: 15, fontWeight: "600" },
        ...
        infoText: { fontSize: 12, color: theme.secondary, flex: 1 },
        amount: { fontSize: 15, fontWeight: "700", color: theme.heading },
        slot: { fontSize: 12, color: theme.secondary },
        divider: {
          height: 1,
          backgroundColor: theme.border,
          marginVertical: 10,
        ...
        // --- Multi-use Form Styles (Used in Fee and Registration Modals) ---
        modalOverlay: {
          flex: 1,
        ...
        // --- Slide-up Picker/Form Modal Styles ---
        pickerOverlay: {
          flex: 1,
